...
import 'package:tikar/utils/form/rent_form.dart';
import 'package:tikar/extensions/extensions.dart';
import 'package:tikar/utils/tables/rent_table.dart';
...
class _RentState extends State<Rent> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<RentCubit, BaseState<List<RentModel?>?>>(
      listener: (context, state) {
        if (state is Valid) {
          context.read<RentCubit>().fetch();
        }
      },
      ...
      builder: (BuildContext context, state) {
        return switch (state) {
          Initial() => Builder(builder: (_) {
              context.read<RentCubit>().getData();
  ...
  Widget body(BuildContext context, List<RentModel?>? _cubit) {
    return Scaffold(
                  ...
                  Center(
                    ...
                    children: [
                      ...
                      Container(
                          ...
                          child: RentPaginatedSortableTable(
                            data: _cubit,
                          ))
                    ],
                  ))
          ...
          Row(
            children: [
              ...
              Padding(
                ...
                child: IconButton(
                    onPressed: context.read<RentCubit>().fetch,
  ...
  CustomCartHeader cardHeader(List<RentModel?> data) {
    return CustomCartHeader(
      data: [
        data.where((e) => e!.active == true).toList().length,
        data.length,
        data.where((e) => e!.basement != null).toList().length,
        data.where((e) => e!.asset != null).toList().length,
      ],
